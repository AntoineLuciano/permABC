...
print("permABC SMC...")
out_perm_smc = perm_abc_smc(key = subkey, model = model, n_particles= N_smc, epsilon_target= 0, y_obs = y_obs, kernel = kernel, verbose = 1, Final_iteration= 0, update_weights_distance= False, stopping_accept_rate= .015)
...
from over_sampling import perm_abc_smc_os
M0s = np.array([ 2*K,  5*K, 7*K, 10*K, 15*K, 20*K], dtype=int)
epsilons_os = []
alphas_M = [.95, .75]
dico_os = {}
N_os = 1000
...
plt.plot(N_sim_perm_smc, epsilon_perm_smc, label="permABC-SMC", linestyle="-", marker="o")
...
plt.legend()
plt.savefig("figures/perm_vs_osum_2outliers.png")
plt.close()
