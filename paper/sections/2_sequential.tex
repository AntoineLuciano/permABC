We present three complementary strategies for accelerating ABC via permutations within an SMC framework.
Section~\ref{sec:permabc_smc} embeds the permABC criterion of Section~\ref{secabc_to_permabc} into the adaptive ABC-SMC algorithm of \citet{delmoralAdaptiveSequentialMonte2012}.
Section~\ref{sec:permABC_OS} introduces \emph{Over-Sampling}, which simulates $M > K$ components per particle and selects the $K$ best-matching ones, while Section~\ref{sec:undermatching} presents \emph{Under-Matching}, which begins with $L < K$ components and progressively increases $L$ toward $K$.
Both extensions exploit the permutation structure to improve the acceptance rate at each SMC iteration.

\subsection{Sequential ABC: state of the art and limitations}
\label{sec:abc_smc}
\old{Rejection-based ABC methods, including the enhanced permABC, remain computationally expensive due to their reliance on the prior as a proposal distribution.
...
A notable feature of this algorithm is the possibility to simulate \( M_{\mathrm{rep}} \) datasets \( \simdata^{(1)}, \dots, \simdata^{(M_{\mathrm{rep}})} \) per parameter value \( \param \).
While increasing \( M_{\mathrm{rep}} \) can reduce variance in the acceptance decision, setting \( M_{\mathrm{rep}} = 1 \) leads to better computational efficiency, as shown in practice by \citet{delmoralAdaptiveSequentialMonte2012} and in theory by \citet{bornn2017use}.
With $M_{\mathrm{rep}}=1$, the importance weights become binary: they are equal to 1 if the particle is accepted (i.e., the distance is below \( \varepsilon \)) and 0 otherwise.
...
\new{Rejection-based ABC methods remain computationally expensive due to their reliance on the prior as proposal. ABC-SMC algorithms \citep{sisson2007sequential, beaumontAdaptiveApproximateBayesian2009b, delmoralAdaptiveSequentialMonte2012} address this by constructing a sequence of intermediate pseudo-posteriors \( \pi_{\varepsilon_0}, \dots, \pi_{\varepsilon_T} \) with decreasing thresholds, combined with importance sampling and MCMC moves. We adopt the adaptive framework of \citet{delmoralAdaptiveSequentialMonte2012} with \( M_{\mathrm{rep}}=1 \) dataset per proposal \citep{bornn2017use}, yielding binary weights. Since the ESS then equals the number of surviving particles and may not reflect true sample diversity, we also monitor the \emph{unique particle rate}---the proportion of distinct parameter values among accepted particles---as a complementary diagnostic.}

\subsection{The permABC-SMC algorithm}
\label{sec:permabc_smc}
...
\new{\subsection{Fast assignment via warm-started swaps}
\label{sec:smart_swap}
Solving the exact linear sum assignment problem \eqref{eqopti} costs \(\mathcal{O}(K^3)\) per particle at every MCMC step, which can dominate the overall runtime in sequential schemes. However, within an SMC iteration the particle configuration varies slowly: the random-walk kernel perturbs the local parameters by a small amount and the tolerance \(\varepsilon_t\) decreases gradually. As a consequence, the optimal permutation \(\sigma^*\) of a proposed particle is typically close to the permutation already associated with its current state.

This observation motivates a warm-started refinement strategy: at each proposed move, we initialise the assignment with the current \(\sigma^*\) and apply a few sweeps of pairwise swaps, each sweep running in \(\mathcal{O}(K^2)\). Swaps are accepted greedily whenever they decrease the permuted distance, following the construction of \citet{bernton2019approximate} but warm-started from the previous optimum rather than from a Hilbert ordering. In practice, one or two sweeps are sufficient to recover \(\sigma^*\) or a permutation inducing the same acceptance decision, so that an exact LSA call is only required at the first SMC iteration or when large jumps occur. This reduces the amortised per-step assignment cost from \(\mathcal{O}(K^3)\) to \(\mathcal{O}(K^2)\) without altering the target pseudo-posterior.}
...
\new{This pushforward representation also yields an asymptotic description of the regime \( M \to \infty \). Let \( g_{\globparam}(z) := \int g(z \mid \globparam,\mu)\,\pi(d\mu) \), \( S_{\globparam} := \operatorname{supp}(g_{\globparam}) \), and define the support-based discrepancy \( \delta_{\globparam}(\obsdata) := \inf_{x_{1:K}\in S_{\globparam}^K} d(\obsdata,x_{1:K}) \). Under mild regularity assumptions (see Appendix~\ref{appendix_os_asymptotics} for the full derivation), the marginal Over-Sampling pseudo-posterior on the global parameter satisfies}
\begin{equation}
    \label{eq:os_pushforward_limit}
    ...
    \right\}.
\end{equation}
\new{In particular, if \( \delta_{\globparam}(\obsdata)=0 \) for all \( \globparam \) in the prior support, then \( \tilde{\pi}_\varepsilon^M(\globparam\mid\obsdata) \to \pi(\globparam) \): infinite Over-Sampling destroys all information on the global parameter. By contrast, the pushforward map \( \Psi_{\obsdata}^{(M)} \) retains the \(K\) matched local parameters, which become concentrated as \(M\) grows. This regime does not commute with the \(K\to\infty\) asymptotics of Section~\ref{sec:asymptotics_K}: Over-Sampling is best viewed as an algorithmic bridge rather than an asymptotic regime to push. Figure~\ref{fig:os} illustrates both effects on the Gaussian toy model.}
...
\new{This method converges in fewer iterations but raises two practical challenges: (i)~calibrating \(M_0\) and \(\varepsilon\) jointly to ensure a large enough initial population, and (ii)~mitigating particle extinction when \(M_t\) decreases, since previously matched compartments may no longer be available. We address both through a duplication mechanism and adaptive heuristics for choosing \((M_t)\); details are in Appendix~\ref{appendix_os}.} 

\begin{figure}[htbp]
    \centering
    \includegraphics[scale=.5]{fig/fig2.pdf}
    \caption{\old{Evolution of the pseudo-posterior \( \pi_\varepsilon^{*M} \) as \( M \) decreases from \( M_0 = 150 \) (yellow) to \( M_T = K = 10 \) (purple) for a Gaussian toy model.}\new{Over-Sampling pushforward marginals for the Gaussian toy model (\( K=10 \)). Each curve corresponds to one \( M \in \{10,\dots,250\} \). Left: marginal on \( \sigma^2 \) flattens towards the prior as \( M \) grows, in agreement with~\eqref{eq:os_pushforward_limit}. Right: marginal on \( \mu_1 \) concentrates as the optimal permutation selects the best match among \( M \) candidates.}}
    \label{fig:os}
\end{figure}
...
\new{As with Over-Sampling, calibrating \(\varepsilon\) relative to \(L_0\) is critical: if the model is misspecified for even one compartment, the final transition \(L_{T-1}=K-1 \to L_T=K\) can cause severe particle degeneracy. Practical guidelines for choosing \((L_t)\) are in Appendix~\ref{appendix_um}; the benefits of Under-Matching in the presence of outliers are demonstrated in Section~\ref{sec:osum}.}
...
\new{For Over-Sampling and Under-Matching, the same alignment principle applies: matched compartments reuse the aligned variance \(\tau_t^2\), while unmatched ones receive a default value \(\tau_0^2 := \max_{k\in\mathcal{I}} \tau_t^2[k]\). In the Under-Matching case, the partial permutation \(\tilde\sigma_i^{-1}\circ\sigma_i\) recovers the correspondence between observed and simulated compartments for each particle. Full details are in Appendix~\ref{appendix_os}.}
